...
% For anat_wake, filtered subject sub18
function csv_filt_sleep()
...
sublabel = readtable('subs_labels.csv',opts);
     ...
     awdir = strcat('/rds/project/tb419/rds-tb419-bekinschtein/Yingge/LZ/EEG/Passive/anat_wake/',subj,'/filtered/');
    
    % Add path to the computed LZ data versus epochs
    addpath(awdir);
    
    dataaw = strcat('EEG_Passive_anat_wake_epoch_0_4_clean_filt_4-13_',subj,'.mat');
    load(dataaw,'LZ_HGSN');
    ...
    % LZ values
    saw = size(sN2,1); % aas: alert & anat_sleep;

    % Add six new columns to each csv for storing corresponding LZ values
    zerosaw = zeros(saw,1);
    sN2 = addvars(sN2,zerosaw,zerosaw,zerosaw,zerosaw,zerosaw,zerosaw,'NewVariableNames',{'FP','F','Cent','Anpos','Parie','Occi'});
   ...
   % and anat_wake
   save_saw = strcat(awdir,'/',subj,'_aw_filt_4-13_sleep.csv');
   writetable(sN2,save_saw);
