...
for ID = 7:32
    % Get subject string first
     ...
     asidx = strcmp(subtable.dataset,'anat_sleep');
     subas = subtable(asidx,:);
     idxnum = ~isnan(subas.clean_EEG_epoch_number); % Filter for subject and clean epochs
     subclean = subas(idxnum,:);
    ...
    data = strcat('EEG_Passive_anat_sleep_epoch_0_4_clean_',subj,'.mat');
    load(data,'LZ_HGSN');
    ...
    savedir = strcat('/rds/project/tb419/rds-tb419-bekinschtein/Yingge/LZ/EEG/Passive/anat_sleep/Reshaped/',subj,'_as_reshape_clean.csv');
